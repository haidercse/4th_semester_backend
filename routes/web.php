...
Route::post('/checkout-store', [FormController::class, 'checkoutStore'])->name('checkout.store');
Route::get('/checkoutList', [WorkshopController::class, 'checkoutList'])->name('checkoutList');

// ফর্ম সাবমিট করার জন্য POST রাউট
Route::post('/book-workshop', [FormController::class, 'bookStore'])->name('workshop.store');
...
Route::get('/gallery_shop', function () {
    return view('frontend.gallery_shop');
})->name('gallery_shop');

Route::get('/plan_visit', function () {
    return view('frontend.plan_visit');
})->name('plan_visit');

Route::get('/contact_support', function () {
    return view('frontend.contact_support');
